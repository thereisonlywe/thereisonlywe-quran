...
public class QuranicText implements Comparable<QuranicText>
{
	...
	public String getVerseContent(int v)
	{
		...
		if (f.exists() || QuranicTextManager.updateText(textIdentifier.getID()))
		{
			return IOEssentials.fetchLine(f, v + 1).replace(
				StringEssentials.THEREISONLYWE_LINE_SEPARATOR, "\n");
		}
		else return "";
	}
	...
	public int hashCode()
	{
		...
		result = prime * result
			+ ((textIdentifier == null) ? 0 : textIdentifier.hashCode());
		return result;
	}
	...
	private void parseText(String text)
	{
		...
		while (counter < text.length())
		{
			if (text.charAt(counter) == ';')
			{
				if (field == 0)
				{
					textIdentifier = QuranicTextManager.getTextIdentifier(Integer
						.parseInt(new String(text.substring(previousindex, counter))));
		...
		if (field != 0)
		{
			order = Byte.valueOf(new String(text.substring(previousindex,
				text.length())));
		}
	}
	...
	{
		if (qt.getOrder() == this.getOrder()) return new String(this
			.getIdentifier().getLanguageCode()
			+ "-"
			+ this.getIdentifier().getAuthor() + "-" + this.getIdentifier().getType())
			.compareTo(new String(qt.getIdentifier().getLanguageCode() + "-"
				+ qt.getIdentifier().getAuthor() + "-" + qt.getIdentifier().getType()));
		else
		{
